...
export const parseProviderProfile = (text: string) => {
  ...
  const baseRate = ratesMatch ? parseFloat(ratesMatch[1].replace(/,/g, '')) : undefined;
...
export const parseRFP = (text: string) => {
  ...
  const budgetMatch =
    text.match(/(?:Budget|Estimated Budget|Total Budget|Budget Range)[:\s]*\$?\s*([\d.,]+)/i) ||
    text.match(/(?:£|€|\$)\s*([\d,]+)/);

  // deadline: common labels or "due by" constructs
  const deadlineMatch =
    text.match(/(?:Deadline|Submission Deadline|Due Date|Due by)[:\s]*([0-9]{4}-[0-9]{2}-[0-9]{2}|[A-Za-z0-9 ,]+)/i) ||
    text.match(/(due by|submit by)\s*([A-Za-z0-9 ,]+)/i);

  // client name heuristics
  const clientMatch =
    text.match(/(?:Client|Client Name|Company|Organization)[:\s]*([A-Za-z0-9 .&-]+)/i);
  ...
  if (reqHeadingMatch && reqHeadingMatch[1]) {
    ...
    const bullets = text
      ...
      .filter((l) => l.match(/^[-•\u2022]|\d+\./));
    if (bullets.length) {
      requirements = bullets.map((b) => b.replace(/^[-•\u2022]\s*/, '').replace(/^\d+\.\s*/, '')).slice(0, 50);
    }
  }
